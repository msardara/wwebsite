-- Track whether a guest group has submitted their RSVP
ALTER TABLE guest_groups
  ADD COLUMN rsvp_submitted BOOLEAN NOT NULL DEFAULT FALSE;

-- Backfill: any group that already has at least one guest with a location
-- has clearly gone through the RSVP submission flow
UPDATE guest_groups
SET rsvp_submitted = TRUE
WHERE id IN (
  SELECT DISTINCT guest_group_id
  FROM guests
  WHERE cardinality(attending_locations) > 0
);

-- Re-create save_rsvp to set rsvp_submitted = TRUE on submission
